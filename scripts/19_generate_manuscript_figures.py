...
for name,row in pivot.sort_values('validation',ascending=False).iterrows():
    c=model_colors[name]; ax.annotate('',xy=(row.temporal_test,row.validation),xytext=(row.validation,row.validation),arrowprops=dict(arrowstyle='-|>',lw=1.6,color=c)); ax.scatter(row.validation,row.validation,s=48,facecolor='white',edgecolor=c,lw=1.4); ax.scatter(row.temporal_test,row.validation,s=58,color=c,zorder=3)
lo=min(pivot.min())-.25; hi=max(pivot.max())+.35; ax.plot([lo,hi],[lo,hi],ls='--',lw=.8,color='#777'); ax.set(xlim=(lo,hi),ylim=(lo,hi),xlabel='Temporal-test RMSE (°C)',ylabel='Validation RMSE (°C)',title='(a) Validation-to-temporal transfer'); ax.legend(handles=[mpl.lines.Line2D([],[],marker='o',ls='-',color=model_colors[n],label=n.replace('_',' ')) for n in pivot.index],frameon=False,fontsize=6.2,loc='upper left')
ax=fig.add_subplot(gs[0,1])
for pid,row in pair.sort_values('model_rmse_c').iterrows():
    c=COL[row.city]; ax.plot([0,1],[row.model_rmse_c,row.climatology_rmse_c],color=c,lw=1.35,alpha=.85); ax.scatter([0,1],[row.model_rmse_c,row.climatology_rmse_c],s=34,color=c,edgecolor='white',lw=.45,zorder=3)
...
for _,r in sp.iterrows():
    ax.plot([0,r.mean_interval_width_c],[r.interval_coverage,r.interval_coverage],color=COL[r.city],lw=1.2,alpha=.75); ax.scatter(r.mean_interval_width_c,r.interval_coverage,s=65,marker='s' if r.thermal_sensor=='Landsat' else 'o',color=COL[r.city],edgecolor='white',zorder=3)
...
city_handles=[mpl.lines.Line2D([],[],marker='o',ls='-',color=c,label=k) for k,c in COL.items()]
sensor_handles=[mpl.lines.Line2D([],[],marker='s',ls='',color='#555',label='Landsat'),mpl.lines.Line2D([],[],marker='o',ls='',color='#555',label='ECOSTRESS')]
ax.legend(handles=city_handles+sensor_handles,frameon=False,ncol=2,fontsize=5.7,loc='lower left')
...
pids=list(meta.sort_values(['city','thermal_sensor']).index); fig,axs=plt.subplots(len(pids),4,figsize=(8.5,15.5),constrained_layout=True)
...
err_all=np.concatenate([np.abs(rasters[p]['predicted_c']-rasters[p]['observed_c'])[rasters[p]['valid_mask'].astype(bool)] for p in pids]); err_max=np.nanpercentile(err_all,98)
unc_all=np.concatenate([rasters[p]['tta_std_c'][rasters[p]['valid_mask'].astype(bool)] for p in pids]); unc_max=np.nanpercentile(unc_all,98)
atlas_ims={}
for i,pid in enumerate(pids):
    ...
    arrays=[obs,pred,err,unc]; cmaps=['inferno','inferno','magma','viridis']; ranges=[(vmin,vmax),(vmin,vmax),(0,err_max),(0,unc_max)]
    for j,(arr,cmap,rr) in enumerate(zip(arrays,cmaps,ranges)):
        im=axs[i,j].imshow(arr,cmap=cmap,vmin=rr[0],vmax=rr[1]); atlas_ims[j]=im; axs[i,j].set_xticks([]); axs[i,j].set_yticks([])
...
fig.colorbar(atlas_ims[0],ax=axs[:,:2],fraction=.018,pad=.012,label='Land-surface temperature (°C)')
fig.colorbar(atlas_ims[2],ax=axs[:,2],fraction=.030,pad=.018,label='Absolute error (°C)')
fig.colorbar(atlas_ims[3],ax=axs[:,3],fraction=.030,pad=.018,label='TTA standard deviation (°C)')
