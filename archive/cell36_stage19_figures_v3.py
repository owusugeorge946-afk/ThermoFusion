...
ax=fig.add_subplot(gs[0,0]); norm=perf.set_index('model').copy(); norm['MAE skill']=1-norm.MAE/norm.MAE.iloc[0]; norm['RMSE skill']=1-norm.RMSE/norm.RMSE.iloc[0]; norm['R²']=norm.R2; norm['|bias| penalty']=-norm.Bias.abs()/norm.Bias.abs().max(); radar=norm[['MAE skill','RMSE skill','R²','|bias| penalty']]
...
ax.set_xticks(angles[:-1],radar.columns); ax.set_ylim(-1.05,1); ax.set_title('(a) Normalized multimetric skill',pad=18); ax.legend(frameon=False,bbox_to_anchor=(.5,-.18),loc='upper center',fontsize=7)
...
for name,row in pivot.iterrows(): ax.plot([0,1],[row.validation,row.temporal_test],'-o',lw=1.7,label=name.replace('_',' ')); ax.text(1.02,row.temporal_test,f'{row.temporal_test:.2f}',va='center',fontsize=7)
...
for pid,row in pair.iterrows(): ax.plot([0,1],row.values,'-o',alpha=.75,lw=1,color=COL.get(meta.loc[pid,'city'],'#555') if 'meta' in globals() and pid in meta.index else '#557A95')
...
ax=fig.add_subplot(gs[1,2]); scene=s17.query("model_split=='temporal_test'"); sc=ax.scatter(scene.rmse_c,scene.r2,c=scene.interval_coverage,s=80+180*scene.median_tta_std_c/scene.median_tta_std_c.max(),cmap='viridis',vmin=0,vmax=1,edgecolor='white');
for _,r in scene.iterrows(): ax.annotate(r.pilot_id,(r.rmse_c,r.r2),xytext=(3,3),textcoords='offset points',fontsize=6)
ax.axhline(0,color='#777',lw=.8); ax.set(xlabel='Scene RMSE (°C)',ylabel='Scene R²',title='(f) Accuracy–skill–coverage landscape'); fig.colorbar(sc,ax=ax,label='90% interval coverage')
