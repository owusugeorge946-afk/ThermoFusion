...
for required_path in [CHIP_DIR, TASK_REGISTER, REVISED_MANIFEST]:
...
all_tifs = sorted(CHIP_DIR.glob("*.tif"))
unexpected_tifs = []
expected_paths = set()
records = []
...
def matching_files(pattern):
    return sorted(CHIP_DIR.glob(pattern), key=lambda p: p.stat().st_mtime, reverse=True)
...
for number, row in enumerate(register.itertuples(index=False), start=1):
    predictor_matches = matching_files(f"{row.prefix}_predictors_10m*.tif")
    thermal_matches = matching_files(f"{row.prefix}_thermal_native*.tif")
...
unexpected_tifs = [path for path in all_tifs if path not in expected_paths]
...
file_count_check = len(all_tifs) == EXPECTED_FILES
...
verdict = [
    ...
    f"Expected GeoTIFF files: {len(all_tifs)}/{EXPECTED_FILES}",
    ...
    f"Unexpected GeoTIFF files in Stage 10 folder: {len(unexpected_tifs)}",
